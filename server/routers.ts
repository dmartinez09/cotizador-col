...
export const appRouter = router({
  ...
  quotations: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      ...
      // Coordinadores, gerentes y admins ven todas
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // Obtener configuración de márgenes para determinar el paso de aprobación
    ...
    /**
     * Aprobación escalonada — delegada al servicio centralizado.
     * Verde: coordinador/gerente/admin aprueba directo.
     * Amarillo: coordinadora → gerente (dos pasos).
     * Rojo: gerente/admin directo.
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // Solo el vendedor que creó la cotización (pendiente) o un admin puede eliminarla
        ...
        // Soft delete — no eliminamos items, solo marcamos la cotización como inactiva
    ...
    // Historial de aprobaciones de una cotización
    ...
    // Registrar que se generó/imprimió un PDF
  ...
  users: router({
    ...
    // Lista básica para resolución de nombres (todos los roles autenticados)
    listBasic: protectedProcedure.query(async () => {
      const allUsers = await db.getAllUsers();
      return allUsers.map(u => ({ id: u.id, name: u.name, role: u.role }));
    }),

    // Crear usuario con contraseña (solo admin)
    create: adminProcedure
      .input(z.object({
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // Usar el email como openId para usuarios locales
  ...
  // ===== PDF DOCUMENTS (metadata via tRPC, binarios via Express routes) =====
  pdfDocuments: router({
    // Lista PDFs de una cotización
    listByQuotation: protectedProcedure
      .input(z.object({ quotationId: z.number() }))
      .query(async ({ input, ctx }) => {
        // Vendedores solo acceden a sus cotizaciones
        if (ctx.user.role === "vendedor") {
